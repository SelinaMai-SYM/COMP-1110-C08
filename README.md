...
Finally, the following instruction would be shown in terminal:

`Input the file for output statistics (.txt): `

Input the file name for the output statistics. For example, if you are using the sample input file, input `operation_statistics.txt` and press Enter.

If the program runs successfully, output statistics will be in the provided output file.
- If a run-time error occurs during execution, no output will be in the output file (or it will remain the same as last success execution). There may be invalid data in the input files, invalid input format or no corresponding file. An error message will be shown in terminal.
...
### main.py
This code is the program entry file for restaurant queue simulation. It will read the input files, execute the simulation process and output the statistics. You need to execute this file to run the program.
...
### statistics.py
This file contains the functions for computing simulation statistics and results and write the statistics to the provided output file.
